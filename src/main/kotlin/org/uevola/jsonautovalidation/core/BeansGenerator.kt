...
object BeansGenerator: KLogging() {
        ...
        val elapsed: Duration = kotlin.time.measureTime {
            getControllersToValidate().forEach { controller ->
                getMethodsToValidate(controller).forEach { method ->
                    getParamsToValidate(controller, method)
    ...
    private fun getControllersToValidate(): Set<Class<*>> {
        val controllers = ClassesUtil.getAnnotatedClassesIn(JsonValidationConfig.controllersPackageName, Controller::class.java)
        val restControllers = ClassesUtil.getAnnotatedClassesIn(JsonValidationConfig.controllersPackageName, RestController::class.java)
