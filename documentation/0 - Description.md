...
1. AOT Compilation > Scanning of annotated DTOs (`@JsonValidation`), and automatic generation of JSON schemas + validation beans
...
```kts
implementation("io.github.ugoevola:json-auto-validation:1.2.0")
```

## How does it work?

1. You declare your DTOs using the `@JsonValidation` annotations and your endpoints to be validated with `@Validate`.
2. AOT compilation phase → the library scans all DTOs annotated with `@JsonValidation` and automatically generates the corresponding JSON schemas and Spring validator beans.
3. Runtime → During execution, incoming requests are intercepted before accessing the controller by `JsonSchemaValidationInterceptor`, which:
